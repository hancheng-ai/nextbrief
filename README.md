...
> **The current release is `0.1.0rc5`, and it is a prerelease.** It lives on
...
`v0.1.0rc5` sdist. A `<owner>/homebrew-tap` repository — which would make this
...
`0.1.0rc5`, a prerelease.

| Channel | State |
|---|---|
| Source checkout — `git clone`, `pip install .` | **live** |
| Zipapp built from a checkout | **live** |
| [TestPyPI](https://test.pypi.org/project/nextbrief/) — `pip`, `pipx`, `uv`, `uvx` with an explicit index URL | **live**: `0.1.0rc5`, sdist and wheel |
...
| GitHub release assets — sdist, wheel, `nextbrief.pyz`, `SHA256SUMS` | **live** on [`v0.1.0rc5`](https://github.com/hancheng-ai/nextbrief/releases/tag/v0.1.0rc5), with a build-provenance attestation. Use the tagged URL: `/releases/latest/` skips prereleases |
