<p align="center">
  <img src="https://raw.githubusercontent.com/hancheng-ai/nextbrief/v0.4.2/packaging/icon/nextbrief.svg" alt="" width="96" height="96">
</p>
...
[![Release](https://img.shields.io/badge/release-v0.4.2-blue)](https://github.com/hancheng-ai/nextbrief/releases/tag/v0.4.2)
...
[![License: Apache 2.0](https://img.shields.io/badge/license-Apache%202.0-blue.svg)](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/LICENSE)

**A daily brief across every project you own — where every claim is checked against evidence before it is allowed to print.**

[中文文档 →](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/README.zh.md)
...
Details in [Privacy](#privacy) and [SECURITY.md](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/SECURITY.md).
...
Below is a real run against the [example workspace](https://github.com/hancheng-ai/nextbrief/tree/v0.4.2/examples/workspace) in this
...
[`examples/workspace/state/brief.json`](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/examples/workspace/state/brief.json) — so
...
>
> ```sh
> # 0.4.2 stands in for whichever candidate you mean; there is no "latest"
> # on that index worth asking for
> pipx install --index-url https://test.pypi.org/simple/ "nextbrief==0.4.2"
> ```
>
...
```sh
curl -fsSLO https://github.com/hancheng-ai/nextbrief/releases/download/v0.4.2/nextbrief.pyz
...
```sh
curl -fsSLO https://github.com/hancheng-ai/nextbrief/releases/download/v0.4.2/SHA256SUMS
...
[CONTRIBUTING](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/CONTRIBUTING.md#windows).
...
`v0.4.2` sdist and checks it against a `sha256` in the stanza — a digest that
...
[`packaging/homebrew/nextbrief.rb`](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/packaging/homebrew/nextbrief.rb), so it is
...
This repository is its own marketplace — [`.claude-plugin/marketplace.json`](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/.claude-plugin/marketplace.json)
...
[`tests/test_plugin.py`](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/tests/test_plugin.py) lints every skill body against
...
Reading it is also the point of [`docs/INVENTORY_SCHEMA.md`](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/docs/INVENTORY_SCHEMA.md):
...
| GitHub release assets — sdist, wheel, `nextbrief.pyz`, `SHA256SUMS` | **live** on [`v0.4.2`](https://github.com/hancheng-ai/nextbrief/releases/tag/v0.4.2), with a build-provenance attestation. Documented by tag rather than `/releases/latest/`, which resolves to the newest non-prerelease |
...
in the prompt: **[docs/ARCHITECTURE.md](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/docs/ARCHITECTURE.md)**.
...
- **Write a terminal status.** `done`, `dropped` and `deferred` all take an item off
  the page, and no agent may write one — see [the design contract](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/CONTRIBUTING.md).
...
See [SECURITY.md](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/SECURITY.md).
...
[CHANGELOG.md](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/CHANGELOG.md), which is the record; this table is the index.
...
|---|---|---|
| Unreleased | — | — |
| [0.4.2](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/CHANGELOG.md#042---2026-09-05) | 2026-09-05 | What the candidate below carried, on PyPI, with no changes to the package since it. The candidate existed to watch the minting fix under a released artifact rather than under a checkout, and it did: installed from TestPyPI, `new` on a copy of `examples/workspace` wrote an empty criteria block where the template had written the title as criterion #1, and `check` on that copy reported the item under the new rule. On the workspace the defect was found on, the new rule reported nothing — the same zero measured before it was added. One release-tooling fix rode along: `scripts/bump-version.sh`'s push hint used `--follow-tags`, which skips the lightweight tags this repository uses; the candidate's tag had to be pushed by hand. |
| [0.4.2rc1](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2rc1/CHANGELOG.md#042rc1---2026-09-05) | 2026-09-05 | `new` and `followup --promote` stopped minting the item's title as its only acceptance criterion. A criterion that repeats the title asks "is the title true?", which it always is, so such an item could never be judged done; three live items on a real backlog were in that state and the template kept producing more. `check` gained a fourth criteria rule in the same change — live items with no criteria at all — so the fix does not turn a visibly fake criterion into an invisibly missing one; measured before adding it, zero such items on the backlog it was checked against. Also the README paragraphs on the Homebrew digest, which had asserted the stale half of a two-phase cycle as permanent. Cut as a candidate to watch the minting fix under a released artifact rather than under a checkout. |
| [0.4.1](https://github.com/hancheng-ai/nextbrief/blob/v0.4.1/CHANGELOG.md#041---2026-08-25) | 2026-08-25 | What the candidate below carried, on PyPI, with no code changes since it. The candidate existed to watch the two fixes under a released artifact rather than under a checkout, and it did: installed from TestPyPI and pointed at the workspace the defects were found on, it kept the `next_actions` row citing NA-0060 — an item the cap held back in that same run, `over_new_item_cap` in the rejection list to prove it — while `0.4.0` from PyPI, on the same snapshot and the same `brief.json`, recorded `unresolvable_evidence` for that citation and dropped the row off the page. The second fix has no live instance to show: this workspace sits directly under its projects root under its own name, so the basename and the root-relative spelling are the same string and the two versions cannot disagree there. It is verified by `tests/test_gates.py` and by nothing else. |
| [0.4.1rc1](https://github.com/hancheng-ai/nextbrief/blob/v0.4.1rc1/CHANGELOG.md#041rc1---2026-08-25) | 2026-08-25 | Two defects in the evidence index, both of which turned an honest citation of a backlog item into a dropped claim, and both found on a live workspace rather than by a test. The index was minted from the post-cap listing, so an item `caps.max_new_items_per_run` held back for the run had neither its id nor either of its path spellings in it — while the digest hands stage 2 every active item, so the model was shown an item it could cite and the gate then read the citation as unsupported. It looked intermittent because sensing mints handles for a project's eight most recently changed paths: NA-0066 and NA-0067 resolved that way, and NA-0060 — three days older, equally uncommitted — was dropped from `next_actions`. The second of the two spellings was wrong in its own right wherever the workspace is not a directory sitting directly under the projects root under an unchanged name: it was built from the basename rather than from the path relative to the resolved root, so a nested or renamed workspace indexed a spelling the sensing stage never emits. |
| [0.4.0](https://github.com/hancheng-ai/nextbrief/blob/v0.4.0/CHANGELOG.md#040---2026-08-20) | 2026-08-20 | Everything the three candidates carried, on PyPI, plus one fix the last candidate found. Three candidates because each was cut after the previous one shipped a claim that turned out not to be true: rc1 folded git worktrees into their repository and stopped capping recorded refusals; rc2 added `settle`, versioned `digest.json`, made the daily log say when the ruler changed, and taught the walk that git's storage is not work under any name; rc3 fixed what verifying rc2 found — a lint reading English by a narrower path than Chinese and matching substrings, a declared probe reaching `BRIEF.md` but not `BRIEF.html` under a comment claiming otherwise, and three advertised bounds that survived direct mutation. The fix below came from rc3 in turn: its new tests were the first thing here to fail only on Windows, which is also how it emerged that rc3 had been tagged off a red `main`. Nothing merges until CI is green now, and nothing is tagged off a red `main` — written into CONTRIBUTING rather than remembered. |
| [0.4.0rc3](https://github.com/hancheng-ai/nextbrief/blob/v0.4.0rc3/CHANGELOG.md#040rc3---2026-08-20) | 2026-08-20 | Three things 0.4.0rc2 shipped wrong, each found by verifying a claim rather than the code. The trace lint read English by a narrower path than Chinese and matched substrings, so it flagged a signed contract and a merged pull request while waving through "I understood the biology of it" — `log`, inside *biology*. A declared `evidence_probe` reached `BRIEF.md` and not `BRIEF.html`, under a comment claiming the two renderers took the same path; on a row declaring `evidence: "reported"` that probe is the only machine-checkable evidence left, and every test for the feature read `BRIEF.md` only — replacing the HTML renderer with a function that raises left all of them green. And three of the four bounds the candidate search advertises survived direct mutation with the suite green, including the in-place slice whose own source comment explains that rebinding it prunes nothing. All five are now entries in the mutation manifest, so the harness watches them rather than a person remembering to. |
...
| [0.2.1](https://github.com/hancheng-ai/nextbrief/blob/v0.2.1/CHANGELOG.md#021---2026-08-09) | 2026-08-09 | Everything the candidate below carried, on PyPI. The candidate itself is what earned this row: it ran the release pipeline end to end for the first time and found that the formula digest job — reachable only since the release before it, and never once observed — did all of its work correctly and then failed to open its pull request, because a repository setting the job had never needed until it became reachable was switched off. |
...
2. **Check `schema_version` first, and stop rather than guess if it is not a
   ...
   [`docs/INVENTORY_SCHEMA.md`](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/docs/INVENTORY_SCHEMA.md).
...
never an error, because a warning that fires on work somebody did correctly is
one people learn to scroll past.

**It was measured in Chinese and reads English by a narrower path**, which is
worth knowing before you trust its silence. The bare-word rule is reached only
when the criterion contains CJK; an English one falls through to a fixed
vocabulary, so the vocabulary is the whole net there. That net first shipped
tuned on software work and flagged a signed contract, a merged pull request and
a bank refund — traces more durable than most of the words it did know — so it
has since been widened with the ones that are not files, and matched on whole
words rather than substrings, which had been reading *bio**log**y* as naming a
log. Currently it flags **16 of 161** open criteria on the workspace it was
tuned against. The English sample here is nine criteria, which is too few to
quote a rate from, and where it is quiet you should read that as *unmeasured*
...
[PRIVACY.md](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/PRIVACY.md) states the same thing as a policy, in one page: the two
...
Read [CONTRIBUTING.md](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/CONTRIBUTING.md) first — especially the design contract, the
...
Apache 2.0. See [LICENSE](https://github.com/hancheng-ai/nextbrief/blob/v0.4.2/LICENSE).
