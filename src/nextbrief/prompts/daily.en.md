...
- **You may not move any backlog item into a terminal status.** Never write `status: done` or `status: dropped`. If you believe something is finished, write `proposed_status: done` and let a human confirm it. A false completion is far more damaging than a missed one: it removes the item from view while the work is still undone.
